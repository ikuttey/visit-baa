...
async function saveScheduleRule(event){event.preventDefault();const listing=selectedListing();if(!listing)return;const payload={listing_id:listing.id,day_of_week:Number(document.getElementById('scheduleDay').value),start_time:document.getElementById('scheduleStart').value,end_time:document.getElementById('scheduleEnd').value||null,capacity:Number(document.getElementById('scheduleCapacity').value),valid_from:document.getElementById('scheduleFrom').value,valid_until:document.getElementById('scheduleUntil').value||null,is_active:true};try{const {error}=await state.client.from('listing_schedule_rules').insert(payload);if(error)throw error;event.currentTarget.reset();document.getElementById('scheduleFrom').value=localDateString();setPageMessage(message,'Recurring schedule added. Generate sessions to publish the next 12 months.','success');await loadCalendarData();}catch(error){setPageMessage(message,error.message,'error');}}
